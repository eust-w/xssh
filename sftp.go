...
func (c *Client) Upload(local string, remote string) (err error) {
	...
	if info.IsDir() {
		return c.UploadDir(local, remote, []string{})
	}
	...
	if c.Size(remoteFile) > 1000 {
		...
		if FileExist(localFile) {
			if rsum != "" {
				...
				if lsum == rsum {
					//ztestlog.Trace("sftp: 文件与本地一致，跳过下载！", localFile)
					return nil
				}
				//ztestlog.Trace("sftp: 正在下载 ", localFile)
			}
		}
	}
	if err := os.MkdirAll(filepath.Dir(localFile), os.ModePerm); err != nil {
		// ztestlog.Println(err)
		return err
	}
...
func (c *Client) UploadFile(localFile, remote string) error {
	...
	if info.Size() > 1000 {
		...
		if rsum != "" {
			lsum, _ := Md5File(localFile)
			if lsum == rsum {
				//ztestlog.Trace("sftp: 文件与本地一致，跳过上传！", localFile)
				return nil
			}
			//ztestlog.Trace("sftp: up； ", localFile)
		}
	}
...
func (c *Client) UploadDir(localDir string, remoteDir string, ignoreList []string) (err error) {
	...
	setPath := func(localDir, path, rootDst string) (string, error) {
	...
	//var ignoreFinalDstList []string = make([]string, 0, len(ignoreList))
	//for _, ignore := range ignoreList {
	//	tem, err := setPath(localDir, ignore, rootDst)
	//	if err != nil {
	//		return err
	//	}
	//	ignoreFinalDstList = append(ignoreFinalDstList, tem)
	//}
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		...
		finalDst, err := setPath(localDir, path, rootDst)
		if err != nil {
			return err
		}
		for _, v := range ignoreList {
			if len(path[len(localDir):]) < len(v) {
				continue
			}
			//ztestlog.Trace("xxx", path[len(localDir):len(localDir)+len(v)], v, v == path[len(localDir):len(localDir)+len(v)])
			if v == path[len(localDir):len(localDir)+len(v)] {
				return nil
			}
		}
		...
		if info.IsDir() {
			...
			// ztestlog.Println("MkdirAll", finalDst)
